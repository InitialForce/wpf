    ...
    internal class PriorityQueue<T>
    {
        ...
        public T Dequeue()
        {
            ...
            if(count > 0)
            {
                ...
                // Capture the payload BEFORE RemoveItem hands the node to the per-queue
                // free list — RemoveItem's pool-push step calls PriorityItem.ClearForPool
                // which nulls _data so a long-lived pooled node cannot keep a completed
                // DispatcherOperation rooted. If we read item.Data after RemoveItem
                // returns we'd get default(T) instead of the dequeued operation, and
                // ProcessQueue's `op = _queue.Dequeue();` would be null — which then
                // NREs on the next-line `op._item = null` stamp.
                T data = item.Data;
                RemoveItem(item);
