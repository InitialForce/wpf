...
namespace System.Windows.Threading
{
    ...
    public sealed class Dispatcher
    {
        ...
        public static Dispatcher FromThread(Thread thread)
        {
            ...
            // owner thread (see ctor, line ~1721), and this is the dominant call
            // shape (HwndSubclass.SubclassWndProc, Dispatcher.CurrentDispatcher,
            // InputMethod, HostVisual, SystemResources, and ReaderWriterLockSlim-
            // Wrapper all hit FromThread(Thread.CurrentThread) once per WndProc
            // dispatch / DispatcherObject construction). A non-null cached value
            // is necessarily the Dispatcher for Thread.CurrentThread (because the
            // ctor only ever writes to its own thread's slot), and a null value
            // means no Dispatcher has been created on this thread, in which case
            // the _dispatchers list contains no matching entry either — so we can
            // return without taking _globalLock or walking the list.
            //
            // (The slow path's preamble notes "managed TLS is very expensive" — a
            // true statement in .NET Framework. In modern .NET RyuJIT compiles
