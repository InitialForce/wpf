...
namespace MS.Internal
{
    ...
    internal class CulturePreservingExecutionContext: IDisposable
    {
        ...
        public static void Run(CulturePreservingExecutionContext executionContext, ContextCallback callback, object state)
        {
            ...
            // Save callback/state and a snapshot of the current culture infos
            // directly onto the CPEC instance. This eliminates the per-Run
            // CultureAndContextManager wrapper allocation; CPEC itself is the
            // state object handed to ExecutionContext.Run, and CallbackWrapper
            // recovers the four fields by casting the state back to CPEC.
            ...
            finally
            {
                ...
                // Drop strong references to user-supplied callback/state so the
                // CPEC does not pin them past Run; matches the lifetime of the
                // discarded CultureAndContextManager wrapper before this change.
        ...
        // Per-Run scratch state, inlined here instead of being held by a
        // separate per-Run CultureAndContextManager wrapper. CPEC is itself
        // passed as the 'object state' to ExecutionContext.Run, and
        // CallbackWrapper recovers everything it needs from a single ref.
        // _callback/_state are nulled in Run's finally so user objects do not
        // outlive Run, matching the pre-change wrapper's lifetime.
