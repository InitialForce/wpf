...
namespace MS.Internal
{
    ...
    ///         (other code runs)               // Modifies culture info to $2
    ///         DispatcherOperation is scheduled
    ///             CPEC.Run(callback)          // CPEC saves culture info $2 directly on
    ///                                         // the CPEC instance (no extra allocation)
    ...
    internal class CulturePreservingExecutionContext: IDisposable
    {
        ...
        public static void Run(CulturePreservingExecutionContext executionContext, ContextCallback callback, object state)
        {
            ...
            // Save callback + state + culture info directly on the CPEC instance.
            // CPEC is captured 1:1 with this Run (DispatcherOperation/Dispatcher.ShutdownImpl
            // both do Capture-then-Run-once-then-Dispose), so reusing the instance fields
            // avoids the per-Run allocation of an inner manager object.
        ...
        /// </summary>
        /// <param name="obj">
        ///     The <see cref="CulturePreservingExecutionContext"/> instance carrying the
        ///     real callback, state, and saved culture info (passed as the state argument
        ///     to <see cref="ExecutionContext.Run(ExecutionContext, ContextCallback, object)"/>).
        /// </param>
        ...
        // Per-Run state. Lifted from a former inner CultureAndContextManager class so that
        // CPEC.Run does not allocate an extra wrapper object on every call. CPEC has 1:1
        // lifecycle with each Run (capture-run-dispose), so storing here is safe.
