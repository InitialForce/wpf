#!/usr/bin/env python3
"""scenario-take-open — take-open dotnet-trace capture.

Captures the take-open code path by:
  1. Pre-staging a real .take fixture into the hive (same import-takes
     helper and fixture env vars as spike-9).
  2. Launching MC-CLI, connecting, and navigating to the Analysis screen
     via the full Explorer dialog flow (student → session → take selection).
  3. Starting dotnet-trace just before the open-take double-click — trace
     captures only the take-open work, no startup or navigation noise.
  4. Stopping the trace when VideoSlider.Maximum > 0 is confirmed (take
     loaded) + SETTLE_S settle — same completion signal as spike-9.
  5. Disconnecting MC cooperatively then taskkill in the finally block
     (only the PID we tracked via find_new_pid, never by image name).

Required env vars:
  SCENARIO_RESULT_DIR — directory where the .nettrace file lands
  SCENARIO_NAME       — filename stem for the output, e.g. "take-open"
...
if not _result_dir_raw:
    print(
        ...
        "Usage: SCENARIO_RESULT_DIR=/tmp/out SCENARIO_NAME=take-open "
        "python3 scenario-take-open.py",
        file=sys.stderr,
    )
    sys.exit(1)
...
# Extra settle time after take-loaded signal before stopping the trace.
# Lets post-open lazy work (thumbnail decode, layout pass) surface.
SETTLE_S = 2.0

# Trace upper bound — dotnet-trace self-exits at this wall-clock duration.
# Should be larger than the longest expected take-open time + SETTLE_S + flush.
# Empirically: take-open completes in ~5-15s; 60s gives headroom for slow I/O.
TRACE_DURATION_S = 60
...
# ---------------------------------------------------------------------------
# tasklist helpers (identical pattern to spike-9 and scenario-startup)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# MCP response helpers (identical pattern to spike-9)
# ---------------------------------------------------------------------------
...
def import_takes(hive_dir: Path) -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="perf-take-open-fixture-", dir=str(win_tmp)) as td:
        scratch = Path(td)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(
        prefix="perf-scenario-take-open-",
        ...
        try:
            ...
            # --- Warmup: navigate to Analysis screen with take ready to open ---
            ...
            # Start dotnet-trace just before the open-take double-click so the
            # trace covers the take-open code path without navigation/warmup
            # noise. We use terminate() (stdin newline) to stop early once
            # VideoSlider.Maximum > 0 + SETTLE_S confirm take is loaded.
            ...
            # Maximum is set only after the video clip has been decoded enough
            # to know its frame count, making it the canonical take-loaded signal.
            mark("take_open.wait_for_take_loaded")
            take_load_start = time.perf_counter()
            ...
            while time.monotonic() < take_load_deadline:
                ...
                try:
                    ...
                    pass
                time.sleep(0.5)
            take_load_elapsed = time.perf_counter() - take_load_start
            mark("take_open.take_loaded",
                 ok=take_loaded, slider_max=slider_max,
                 elapsed_s=round(take_load_elapsed, 2))
            ...
            # Settle: let post-open work (thumbnail decode, layout pass) finish.
            ...
            print(f"\n=== scenario-take-open artifacts ({RESULT_DIR}) ===", flush=True)
